	...
	<el-drawer
		...
		<div class="drawer-content">
			<el-form :model="roleForm" :rules="roleRules" ref="roleForm" label-width="120px">
				<el-form-item prop="name" label="角色名称">
					<el-input placeholder="请输入角色名称" v-model="roleForm.name" class="inputWidth" :disabled="roleForm.internal">
					</el-input>
				</el-form-item>
				<el-form-item prop="description" label="角色说明">
					<el-input type="textarea" v-model="roleForm.description" class="inputWidth"></el-input>
				</el-form-item>
		...
		<div class="custom-drawer-footer">
			<div class="flex-row" style="justify-content:space-between">
				...
				<div>
					<el-button type="primary" @click="addSubmit('roleForm')"  class="form-submit">保存</el-button>
				</div>
			</div>
		</div>
	...
	methods: {
		...
		getRoleMenuTree(){
			...
			roleMenuTree({
				...
				let notExistent = filterNotExistent(temp,'id','children',tempArry,[],'down')
		...
		addSubmit(formName){
			this.$refs[formName].validate((valid) => {
				if (valid) {
					this.roleForm.entryId = [].concat(this.$refs.menuListTree.getHalfCheckedKeys(),this.$refs.menuListTree.getCheckedKeys())
					console.log(this.roleForm);

					if(this.roleForm.id){
						editeRole(this.roleForm).then(res => {
							this.$notify.success({
								title: '成功',
								message:'修改角色成功'
							});
