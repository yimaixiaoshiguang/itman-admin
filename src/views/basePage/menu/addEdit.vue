<template>
	<el-drawer
        ...
        size="50%"
		>
		<el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="100px" style="width:80%;" class="mod-menu">
			<el-form-item label="菜单类型" prop="type">
				<el-radio-group v-model="dataForm.type" @change="changeMenuType" :disabled="!!dataForm.id">
					<el-radio v-for="(type, index) in typeList" :label="index" :key="index">{{ type }}</el-radio>
				</el-radio-group>
			</el-form-item>
			<el-form-item label="英文名称" prop="name">
				<mvk-input v-model.trim="dataForm.name" placeholder="英文名称"></mvk-input>
			</el-form-item>
			<el-form-item label="中文名称" prop="title">
				<mvk-input v-model.trim="dataForm.title" placeholder="中文名称"></mvk-input>
			</el-form-item>
			<el-form-item label="上级菜单" prop="parentName" v-show="dataForm.type != 0" :rules="dataForm.type != 0 ? dataRule.parentName : [{required:false}]" >
				<el-popover
					ref="menuListPopover"
			...
			<el-form-item v-show="dataForm.type != 2" label="菜单URL" prop="path" :rules="dataForm.type != 2 ? dataRule.path : [{required:false}]">
				<el-input v-model="dataForm.path" placeholder="菜单URL"></el-input>
			</el-form-item>
			<el-form-item v-show="dataForm.type != 2" label="前端组件" prop="component" :rules="dataForm.type != 2 ? dataRule.component : [{required:false}]">
				<el-input v-model="dataForm.component" placeholder="前端组件"></el-input>
			</el-form-item>
			<el-form-item label="是否路由菜单" prop="isShow" v-show="dataForm.type != 2">
				<el-radio-group v-model="dataForm.isShow">
					<el-radio :label="true">是</el-radio>
					<el-radio :label="false">否</el-radio>
				</el-radio-group>
			</el-form-item>
			<el-form-item label="打开方式" prop="isOpen" v-show="dataForm.type != 2">
							...
							<div class="mod-menu__icon-inner">
								<div class="mod-menu__icon-list">
									<el-button
										...
										<icon-svg :icon="item" size="18"></icon-svg>
									</el-button>
								</div>
	...
	methods: {
		...
		edit(record){
			...
			this.$nextTick(()=>{
				...
				if(this.dataForm.parentId != 0 && !this.dataForm.parentName){
					this.dataForm.parentName = this.menuTreeList.filter(item => item.id == this.dataForm.parentId)[0].title
				}
			})
			this.getMenuTree()
		},
		...
		handleSubmit(formName){
			...
			this.$refs[formName].validate((valid) => {
				if (valid) {
					this.dataForm.id ? editMenu(this.dataForm).then(res => {
						this.$notify.success({
							title: '成功',
							message: '编辑成功'
						});
